...
func (m model) enterNoteMode() (model, tea.Cmd) {
	m.errorMessage = ""
	m.uiMode = uiModeInputNote
	
	// Get the current note from the selected event
	selectedEvent := m.eventsList.SelectedItem().(*eventItem)
	currentNote := ""
	for _, attendee := range selectedEvent.Attendees {
		if attendee.Self && attendee.Comment != "" {
			currentNote = attendee.Comment
			break
		}
	}
	
	m.inputNote.SetValue(currentNote)
	m.inputNote.Focus()
	return m, textinput.Blink
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case uiModeInputNote:
			if msg.String() == "enter" {
				selectedEvent := m.eventsList.SelectedItem().(*eventItem)
				noteText := m.inputNote.Value()
				m.uiMode = uiModeDefault
				return m, tea.Batch(
					m.eventsList.StartSpinner(),
					m.updateEventNote(selectedEvent, noteText),
				)
			}
		}
	}
...
func (m model) updateEventNote(ev *eventItem, noteText string) tea.Cmd {
	return func() tea.Msg {
		ctx := context.Background()

		client, err := calendar.NewService(ctx, option.WithHTTPClient(oauthClient))
		if err != nil {
			return nonFatalErrorMsg{errorMessage: "Unable to retrieve Calendar client: " + err.Error()}
		}

		// Update the attendees comment
		for _, a := range ev.Attendees {
			if a.Self {
				a.Comment = noteText
				break
		...
		if err != nil {
			return nonFatalErrorMsg{errorMessage: "Unable to update event note: " + err.Error()}
		}
